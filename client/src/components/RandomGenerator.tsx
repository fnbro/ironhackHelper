...
export default class RandomGenerator extends Component<IProps, IState> {
    ...
    render() {
        ...
        window.CS.getBMState().members.filter(user => user.isMember as any === true).forEach((user, ind, arr) => {
            if (ind % 2)
        ...
        if (window.CS.getUIState().currentUser.isAdmin || window.CS.getUIState().currentUser.isMember) {
            ...
            <button className="randomBtn" onClick={this.randomizeAllUsers}>Random</button>
                    {window.CS.getBMState().members.length > 0 && <h2 id="memberDescription">You have {window.CS.getBMState().members.filter(user => user.isMember as any === true).length} members in your Bootcamp</h2>}
